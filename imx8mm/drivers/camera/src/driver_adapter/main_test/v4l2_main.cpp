...
enum v4l2_memory g_memoryType = V4L2_MEMORY_MMAP;

const char sleep_time = 2;
    ...
    const std::shared_ptr<FrameSpec>& buffer)
{
    ...
    if (buffer->bufferPoolId_ == 0) {
        sprintf_s(path, sizeof(path), "./imx8mm%d.yuv", index);
    }
...
void V4L2SetDeviceFormat(DeviceFormat& format, const std::string devname)
{
    ...
    constexpr uint32_t capturemode = 0;
    constexpr uint32_t numerator = 1;
    constexpr uint32_t denominator = 30;

    if (devname == "uvcvideo" || devname == "uvcvideo1" || g_devNameUvc == "0") {
        if (g_isPreviewOnUvc) {
    ...
    format.fmtdesc.capturemode = capturemode;
    format.fmtdesc.fps.numerator = numerator;
    format.fmtdesc.fps.denominator = denominator;
}
    ...
    const std::shared_ptr<HosV4L2Dev>& myV4L2Dev)
{
    ...
    rc = myV4L2Dev->ConfigSys(devname, CMD_V4L2_SET_FPS, format);
    if (rc == RC_ERROR) {
        return rc;
    }

    rc = myV4L2Dev->ReqBuffers(devname, g_bufCont, g_memoryType);
    if (rc == RC_ERROR) {
        return rc;
    }
