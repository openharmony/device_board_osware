...
static bool fsl_sai_rx_irq(int irq, void *devid)
{
...
static bool fsl_sai_readable_tfr_reg(unsigned int reg)
{
    bool ret = false;
    switch (reg) {
        case FSL_SAI_TFR0: fallthrough;
        case FSL_SAI_TFR1: fallthrough;
        case FSL_SAI_TFR2: fallthrough;
        case FSL_SAI_TFR3: fallthrough;
        case FSL_SAI_TFR4: fallthrough;
        case FSL_SAI_TFR5: fallthrough;
        case FSL_SAI_TFR6: fallthrough;
        case FSL_SAI_TFR7:
            ret = true;
            break;
...
static bool fsl_sai_readable_rxr_reg(unsigned int reg)
{
    bool ret = false;
    switch (reg) {
        case FSL_SAI_RDR0: fallthrough;
        case FSL_SAI_RDR1: fallthrough;
        case FSL_SAI_RDR2: fallthrough;
        case FSL_SAI_RDR3: fallthrough;
        case FSL_SAI_RDR4: fallthrough;
        case FSL_SAI_RDR5: fallthrough;
        case FSL_SAI_RDR6: fallthrough;
        case FSL_SAI_RDR7: fallthrough;
        case FSL_SAI_RFR0: fallthrough;
        case FSL_SAI_RFR1: fallthrough;
        case FSL_SAI_RFR2: fallthrough;
        case FSL_SAI_RFR3: fallthrough;
        case FSL_SAI_RFR4: fallthrough;
        case FSL_SAI_RFR5: fallthrough;
        case FSL_SAI_RFR6: fallthrough;
        case FSL_SAI_RFR7: fallthrough;
        case FSL_SAI_RMR:
            ret = true;
            break;
...
static bool fsl_sai_readable_reg(struct device *dev, unsigned int reg)
{
    ...
    if (fsl_sai_readable_tfr_reg(reg) == true) {
        return true;
    }

    if (fsl_sai_readable_rxr_reg(reg) == true) {
        return true;
    }

    switch (reg) {
        case FSL_SAI_TMR: fallthrough;
        case FSL_SAI_MCTL: fallthrough;
        case FSL_SAI_MDIV: fallthrough;
        case FSL_SAI_VERID: fallthrough;
        case FSL_SAI_PARAM: fallthrough;
        case FSL_SAI_TTCTN: fallthrough;
        case FSL_SAI_RTCTN: fallthrough;
        case FSL_SAI_TTCTL: fallthrough;
        case FSL_SAI_TBCTN: fallthrough;
        case FSL_SAI_TTCAP: fallthrough;
        case FSL_SAI_RTCTL: fallthrough;
        case FSL_SAI_RBCTN: fallthrough;
        case FSL_SAI_RTCAP:
            return true;
        default:
            return false;
...
int32_t SaiSetHwParamsPin(struct fsl_sai *sai, u32 channels, u32 *pins, u32 *slots)
{
    int32_t ret = 0;
...
int32_t SaiSetHwParamsClk(struct fsl_sai *sai, bool tx, u32 bclk, const enum AudioStreamType streamType)
{
    int32_t ret = 0;
...
u32 SaiGetdlMask(struct fsl_sai *sai, bool tx, u32 pins)
{
    ...
    if (dl_cfg_idx >= dl_cfg_cnt) {
        AUDIO_DRIVER_LOG_ERR("fsl,dataline%s invalid or not provided.", sai->is_dsd ? ",dsd" : "");
        return HDF_FAILURE;
    }
    ...
    struct fsl_hwParams_register *val, unsigned int dl_mask)
{
    ...
    if (!sai->slave_mode[tx]) {
        if (!sai->synchronous[TX] && sai->synchronous[RX] && !tx) {
            regmap_update_bits(sai->regmap, FSL_SAI_TCR4(offset),
                ...
                FSL_SAI_CR4_CHMOD_MASK, val->cr4);
            regmap_update_bits(sai->regmap, FSL_SAI_TCR5(offset),
                FSL_SAI_CR5_WNW_MASK | FSL_SAI_CR5_W0W_MASK |
                FSL_SAI_CR5_FBT_MASK, val->cr5);
    ...
    if (sai->dataline != 0x1) {
        if (dl_mask <= 1 || sai->is_multi_lane) {
    ...
    regmap_write(sai->regmap, FSL_SAI_xMR(tx), val->mr);
...
int32_t SaiSetHwParams(const struct PlatformData *pd, const enum AudioStreamType streamType)
{
    ...
    ret = SaiSetHwParamsPin(sai, channels, &pins, &slots);
    if (ret != HDF_SUCCESS) {
        ADM_LOG_ERR("SaiSetHwParamsPin fail ret=%d", ret);
        return ret;
    }
    ...
    ret = SaiSetHwParamsClk(sai, tx, bclk, streamType);
    if (ret != HDF_SUCCESS) {
        ADM_LOG_ERR("SaiSetHwParamsClk fail ret=%d", ret);
        return ret;
    }

    val.cr5 = SaiGetHwParamsCR5(sai, slot_width, word_width);
    val.cr4 = SaiGetHwParamsCR4(sai, slot_width, slots);
    dl_mask = SaiGetdlMask(sai, tx, pins);
    if (__sw_hweight8(dl_mask & 0xFF) < pins) {
        AUDIO_DRIVER_LOG_ERR("channel not supported");
        return -EINVAL;
    }
    for (i = 0; i < BYTE_NUM; i++) {
        trce_mask = (1 << (i + 1)) - 1;
        if (__sw_hweight8(dl_mask & trce_mask) == pins) {
            break;
        }
    }
...
static int32_t HandleDaiMode(unsigned int fmt, int32_t *val_cr2, int32_t *val_cr4, struct fsl_sai **sai)
{
    /* DAI mode */
...
static int32_t HandleDaiCLK(unsigned int fmt, int32_t *val_cr2, int32_t *val_cr4)
{
    /* DAI clock inversion */
...
static int32_t SaiSetDaiFmtTr(struct PrivPlatformData *ppd, unsigned int fmt, int fsl_dir)
{
    ...
    if (HandleDaiMode(fmt, &val_cr2, &val_cr4, &sai) < 0) {
        return -EINVAL;
    }

    if (HandleDaiCLK(fmt, &val_cr2, &val_cr4) < 0) {
        return -EINVAL;
    }
...
#define NUM_128  (128)
#define DELAY_10 (10)
int32_t SaiDriverInitClk(struct PrivPlatformData *ppd, void __iomem *base)
{
    int ret = 0, i = 0;;
...
int32_t SaiDriverInitIrq(struct PrivPlatformData *ppd, struct device_node *np)
{
    int irq = 0, ret = 0;
    struct fsl_sai *sai = &ppd->sai;
...
{
    struct PrivPlatformData *ppd = (struct PrivPlatformData *)pd->dmaPrv;
    struct device_node *np = ppd->pdev->dev.of_node;
    ...
    ret = SaiDriverInitClk(ppd, base);
    if (ret != HDF_SUCCESS) {
        ADM_LOG_ERR("SaiDriverInitClk fail ret=%d", ret);
        return ret;
    }
    sai->dataline = 0xFF;
...
void SaiTriggerStart(struct fsl_sai *sai, bool tx, u8 channels, u32 dl_mask, u32 pins)
{
    unsigned char offset = sai->reg_offset;
    int i = 0, j = 0, k = 0;
    /*
